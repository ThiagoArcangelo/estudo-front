...
import type { FatorOperacional } from "../../models";
...
export function DialogCadFatores(props: ItemProps) {
  const [loading, setLoading] = useState(false);
  const [lista, setLista] = useState<options[]>([]);
  const {
    postFatorOperacionalGravar,
    putFatorOperacionalAtualizar,
    getFatorOperacionalMotivos,
  } = getSedApi();
  ...
  const ListaMotivos = useCallback(async () => {
    try {
      setLoading(true);
      const retorno = await getFatorOperacionalMotivos({
        headers: {
    ...
    async (data) => {
      try {
        ...
        const retorno = await (!props.codigo || props.codigo <= 0
          ? postFatorOperacionalGravar(data as FatorOperacional, {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`,
              },
            })
          : /**ATUALIZAÇÃO FATOR OPERACIONAL */
            putFatorOperacionalAtualizar(data as FatorOperacional, {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`,
              },
            }));
  ...
  return (
    <Dialog open={props.open} onOpenChange={props.setOpenDialog}>
      ...
      <DialogContent className="sm:max-w-[580px] bg-white border-zinc-400 shadow-2xl">
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <DialogHeader>
            <DialogTitle>Cadastrar Fatores Operacionais</DialogTitle>
