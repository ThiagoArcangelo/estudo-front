...
export function DialogCadResposta(props: ItemProps) {
  const [loading, setLoading] = useState(false);
  const { postRespostaGravar, putRespostaAlterar } = getSedApi();
  ...
  const onSubmit: SubmitHandler<FormValues> = useCallback(
    async (data) => {
      try {
        ...
        const retorno = await (!props.codigo || props.codigo <= 0
          ? postRespostaGravar(data as Resposta, {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`,
              },
            })
          : /**ATUALIZAÇÃO MOTIVO DESLIGAMENTO */
            putRespostaAlterar(data as Resposta, {
              headers: {
                Authorization: `Bearer ${localStorage.getItem("token")}`,
              },
            }));
    ...
    [confirmDialog, props, postRespostaGravar, putRespostaAlterar]
  );
            ...
            <div className="grid gap-3">
              ...
              <Input
                ...
                className=" bg-white border-zinc-500 border-[1px] placeholder:normal-case uppercase
                               focus:ring-0 focus-visible:ring-0 focus:bg-none text-zinc-500"
                autoComplete="off"
                maxLength={80}
                placeholder="Informe a descreção..."
