...
const { getPerguntaAvaliacao, deletePerguntaAvaliacaoExcluir } = getSedApi();

export default function CadastroPerguntaAvaliacao() {
  ...
  const ListaPerguntasAvaliacao = useCallback(async (descricao?: string) => {
    try {
      setLoading(true);
      const retorno = await getPerguntaAvaliacao(undefined, {
  ...
  const ExcluirCadFatores = useCallback(
    async (codigo: number) => {
      try {
        ...
        const retorno = await deletePerguntaAvaliacaoExcluir(undefined, {
          params: { codigo },
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
    ...
    [ListaPerguntasAvaliacao, confirmDialog]
  );

  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    ListaPerguntasAvaliacao(descricao.toUpperCase());
  }
    ...
    >
      ...
      {itemSelecionado && (
        <DialogPerguntaAvaliacao
          ...
          onSucess={ListaPerguntasAvaliacao}
        />
      )}
    </div>
  );
